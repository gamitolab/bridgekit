...
`@gamitolab/bridgekit/contract` entrypoint. The id is validated against
...
<Tabs>
  ...
  ```ts
  import { Async, defineContract, State, Stream, Void } from '@gamitolab/bridgekit/contract';
...
`@gamitolab/bridgekit/contract` is a **pure, side-effect-free** entrypoint: `defineContract`,
