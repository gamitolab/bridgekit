...
`@gamitolab/bridgekit/contract`, then walks the `defineContract` AST to extract params/result
...
`@gamitolab/bridgekit/contract` or a relative contract path is a **hard error with `file:line`**.
